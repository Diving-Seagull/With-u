...
class MemberDataSource {
  ...
  final _uriPath = 'http://${IpAddress.IP_PATH}:8080/api/member';
  ...
  Future<Member?> getMemberProfile(String jwtToken, String email) async {
    try {
      TokenDto tokenDto = TokenDto.fromJson(json.decode(jwtToken));
      headers['Authorization'] = 'Bearer ${tokenDto.token}';
      http.Response response =
      await RestApiSession.getUrl(Uri.parse('$_uriPath/$email'), headers);
      final int statusCode = response.statusCode;
      if (statusCode == 200) {
        print(json.decode(utf8.decode(response.bodyBytes)));
        Map<String, dynamic> result =
